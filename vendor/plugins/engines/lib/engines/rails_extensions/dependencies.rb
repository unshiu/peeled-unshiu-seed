...
module Engines::RailsExtensions::Dependencies
  ...
  def require_or_load_with_engine_additions(file_name, const_path=nil)
    ...
    Engines.code_mixing_file_types.each do |file_type| 
      ...
      if file_name =~ /^(.*app\/#{file_type.pluralize}\/)?(.*_#{file_type})(\.rb)?$/
        base_name = $2
        ...
        Rails.plugins.each do |plugin|
          plugin_file_name = File.expand_path(File.join(plugin.root, 'app', file_type.pluralize, base_name))
        ...
        if Engines.disable_application_code_loading
          ...
          app_file_name = File.join(RAILS_ROOT, 'app', file_type.pluralize, "#{base_name}")
